...
// attacker markup from our verified domain to the whole league (audit M4).
...
export function tradeProposedHtml({
  ...
  const oracleBlock = aiAnalysis
    ? `<div style="margin:20px 0;padding:14px 16px;border-left:3px solid #FFD700;background:rgba(255,215,0,0.05);">
        ...
        <p style="margin:0;font-size:14px;line-height:1.6;color:#d4d4e8;font-style:italic;">${esc(aiAnalysis)}</p>
      </div>`
    : "";
  return `<div style="${baseStyle}">
    <p style="${goldStyle}">⚡ Ultimate Fantasy Football</p>
    <h2 style="color:#0057FF;margin:8px 0 16px;">New Trade Offer</h2>
    <p><strong>${esc(proposerTeamName)}</strong> has sent you a trade offer in <strong>${esc(leagueName)}</strong>.</p>
    <p><strong>They offer:</strong> ${esc(proposerPlayers.join(", ")) || "—"}</p>
    <p><strong>They want:</strong> ${esc(receiverPlayers.join(", ")) || "—"}</p>
    ${oracleBlock}
...
export function tradeRespondedHtml({
  ...
  return `<div style="${baseStyle}">
    ...
    <p><strong>${esc(responderTeamName)}</strong> has <strong>${status}</strong> your trade offer in <strong>${esc(leagueName)}</strong>.</p>
...
export function tradeVetoedHtml({
  ...
  return `<div style="${baseStyle}">
    ...
    <p>The commissioner has vetoed your trade in <strong>${esc(leagueName)}</strong>.</p>
    ${reason ? `<p style="color:#d4d4e8;font-style:italic;">"${esc(reason)}"</p>` : ""}
    <p style="margin-top:24px;"><a href="${url}" style="${linkStyle}">View trade history →</a></p>
    <p style="${mutedStyle};margin-top:32px;">Ultimate Fantasy Football · Trade notification.</p>
  </div>`;
}
...
export function waiverResultsHtml({
  ...
  const rejectedRows = rejected.map(
    (b) => `<tr><td style="padding:6px 0;color:#CC0000;">✗ Lost</td><td style="padding:6px 12px;color:#a0a0b8;">${esc(b.playerName)}</td><td style="padding:6px 0;color:#a0a0b8;text-align:right;">$${b.bidAmount}</td></tr>`
  ).join("");
  return `<div style="${baseStyle}">
    <p style="${goldStyle}">⚡ Ultimate Fantasy Football</p>
    <h2 style="color:#0057FF;margin:8px 0 4px;">${esc(leagueName)}</h2>
...
export function announcementHtml({
  ...
  return `<div style="${baseStyle}">
    ...
    <div style="border-left:3px solid #FFD700;padding:12px 16px;background:rgba(255,215,0,0.05);margin-bottom:24px;">
      <p style="margin:0 0 10px;font-weight:700;font-size:16px;">${esc(title)}</p>
...
export function onTheClockHtml({
  ...
  return `<div style="${baseStyle}">
    ...
    <p style="margin: 0 0 16px; color: #f4f4f8;">It's <strong>${esc(teamName)}</strong>'s turn to pick.</p>
...
export function leagueInviteHtml({
  ...
  return `<div style="${baseStyle}">
    ...
    <p style="margin: 0 0 16px; color: #f4f4f8;">
      <strong>${esc(commissionerName)}</strong> has invited you to join <strong style="${goldStyle}">${esc(leagueName)}</strong> on Ultimate Fantasy Football.
    </p>
