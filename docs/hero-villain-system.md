...
- **Draft powers travel with the player on trades.** Once a power is attached to a player at draft time, it is stored on that player's record — not the manager's. If that player is traded, the receiving manager inherits the power bonus for the rest of the season. The original drafter does NOT get the power back. This is permanent and non-reversible: trading away a power-enhanced player means giving up the bonus for good. Power-boosted players are inherently more valuable trade assets.

### Powers Nate has already locked in:

1. **Foresight Coin** — Instead of taking this round's dealt power, the holder looks ahead at THEIR OWN next-two-rounds' powers (rounds N+1, N+2 in their personal order) and picks one to receive now instead. Foresight Coin is then **consumed (one-time use only)**. To maintain the 16-powers-for-16-rounds math, FC physically swaps into the vacated slot (the slot the chosen power just left), but it is flagged as spent. If FC surfaces again in that later round, it does nothing — the manager gets a null round with no power effect. **Chain rule: chaining is impossible.** Each manager has FC exactly once in their shuffle, so peeking at a second FC is not possible. The consumed flag also ensures the spent FC cannot be reactivated if the slot-swap brings it to a later round. **Edge cases:** At round 15, only one round ahead exists (round 16) — FC still works but the peek window is one power instead of two. At round 16, FC is fully wasted — no rounds ahead to peek at, null effect.
...
3. **Draft Heist** — Steal another manager's draft pick slot for this round only (swap positions). Confirmed scope: **just the ONE pick this round** — not any other remaining picks that manager has. **Timing:** must be declared at the start of the round before the first pick is made. **Simultaneous Heist rule:** if two managers both draw Draft Heist the same round and target different people, resolve in original pick order — the manager with the earlier pick slot executes first, then the second heist resolves against the now-modified order. If two managers heist each other (A targets B, B targets A), the heists cancel out and both remain in their original positions. **Shield interaction:** Hero's Shield is granted once per round regardless of how many Heists are active — one Shield total, capable of blocking only one Heist.
4. **Hero's Shield** *(paired with Draft Heist)* — Every time ANY manager's personal shuffle deals them Draft Heist, the system ALSO grants Hero's Shield to one random Hero manager for that same round. If that Hero is the heist target, the steal is auto-blocked. Since (in the personal-shuffle model) everyone eventually draws Draft Heist at some round, this pairing fires once per manager, at a different round each time. **Visibility:** the Hero manager who receives the Shield IS NOTIFIED privately at round start — it appears alongside their own power for that round. It is NOT publicly announced. Other managers learn the Shield existed only if it triggers and blocks a Heist.

### Candidate powers — for your review (status notes per Take Doctor pass):

| # | Name | Effect | Notes |
|---|------|--------|-------|
| 5 | **Iron Defense** | Tied to this round's pick: if it's a D/ST, that D/ST's scoring is doubled, season-long. Fizzles if not a D/ST. **Floor rule:** if the D/ST scores negative in a given week, the score is floored at 0 before doubling — the power can never make a bad week worse, only amplify positive weeks. | KEPT (now tied-to-pick) |
...
| 14 | **Time Stone** | REWORKED: Tied to this round's pick (any position, no fizzle): attaches to the drafted player at draft time, dormant until they get injured during the season. **Freeze activation:** the first week the player is injured (out of game action), the frozen score is calculated using this 3-tier hierarchy — (1) if the injury game score (what they scored the week they got hurt, including partial stats) is ≥ 8 pts, freeze at that score; (2) else if the prior fully healthy game score is ≥ 8 pts, freeze there instead; (3) if neither is ≥ 8, freeze at whichever of the two is higher. **Each week they're injured:** if the manager keeps them in the starting lineup, they score the frozen amount. The moment the manager removes them from the lineup while injured, the freeze breaks permanently — zero scoring resumes for as long as they're out, even if the manager re-adds them. **Return to health:** when the player returns to game action, Time Stone deactivates and they score normally again. | KEPT (reworked). Universal injury-insurance power — real risk/reward each week (ride the frozen score vs. bench and lose it forever). **Implementation note:** requires nightly player sync (injury_status from Sleeper) to distinguish injured weeks from bye weeks. Scoring logic built alongside the nightly sync. |
