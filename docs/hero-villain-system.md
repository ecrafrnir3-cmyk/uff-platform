...
| 14 | **Time Stone** | REWORKED: Tied to this round's pick (any position, no fizzle): if that player gets injured and the manager KEEPS them in the starting lineup, the player continues scoring their **FROZEN SCORE** — locked in at the moment this power is dealt (draft time), based on that player's most recent NFL regular-season game score at that point. If no prior game exists (e.g., a rookie with no NFL games), frozen score defaults to 0. The frozen score does NOT update if the player has healthy games later in the season before a second injury — it is always the draft-time snapshot. The player continues using this frozen score every week until they return to game action. If the manager ever benches them while injured, the freeze is permanently broken — normal (zero) scoring resumes for as long as they're out. | KEPT (reworked). Universal injury-insurance power — real risk/reward each week (ride the frozen score vs. bench and lose it forever) |
