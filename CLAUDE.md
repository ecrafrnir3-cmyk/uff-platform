...
- **⏳ BLOCKED ON DB RESTORE (do first thing once up)**: apply `supabase/migrations/20260817120000_draft_clock_server_side.sql` (add `draft_started_at`, stamp it in `start_draft`, create `force_autopick` mirroring `make_draft_pick`'s insert core — read the live body first); deploy score-matchups edge function (v16); relax `move_to_ir` to accept Out/Doubtful; add season-titles bracket-complete guard; verify `uff_game_schedule` seeded for 2026; dump all RPCs+RLS into `supabase/migrations/`; **THEN `git push`** (the draft page now selects `draft_started_at` — pushing before the column exists breaks the draft page).
